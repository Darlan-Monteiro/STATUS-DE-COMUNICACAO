...
def get_local_edge_version():
    ...
    """
    try:
        # Caminho da chave do registro para o Edge
...
def gerenciar_edgedriver():
    ...
    if os.path.exists("msedgedriver.exe"):
        print("ℹ️ O arquivo msedgedriver.exe já existe. Pulando o download.")
        return True
    ...
    # A página de download do WebDriver
    ...
    try:
        # Usa BytesIO para descompactar o arquivo em memória, sem salvar o .zip no disco
        zip_file = zipfile.ZipFile(BytesIO(driver_response.content))
        # Extrai APENAS o msedgedriver.exe para a pasta atual ('.')
